...
class DeleteCharitableGivingReliefRequestParserSpec extends UnitSpec with MockDeleteCharitableGivingReliefValidator {
  ...
  val inputData: DeleteCharitableGivingTaxReliefRawData = DeleteCharitableGivingTaxReliefRawData(nino, taxYear)

  lazy val parser = new DeleteCharitableGivingReliefRequestParser(mockValidator)

  "parse" should {

    "return a request object" when {
      "valid raw data is supplied" in {
    ...
    "return an ErrorWrapper" when {
      ...
      "a single validation error occurs" in {
        MockDeleteCharitableGivingReliefValidator
          .validate(inputData)
