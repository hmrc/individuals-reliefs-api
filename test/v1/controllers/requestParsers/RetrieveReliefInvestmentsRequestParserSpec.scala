...
class RetrieveReliefInvestmentsRequestParserSpec extends UnitSpec {
  ...
  val inputData: RetrieveReliefInvestmentsRawData = RetrieveReliefInvestmentsRawData(nino, taxYear)
  ...
  "parse" should {
    ...
    "return a request object" when {
      "valid request data is supplied" in new Test {
        ...
        parser.parseRequest(inputData) shouldBe
          Right(RetrieveReliefInvestmentsRequest(Nino(nino), TaxYear.fromMtd(taxYear)))
      }
    }
    ...
    "return an ErrorWrapper" when {
      ...
      "a single validation error occurs" in new Test {
        MockRetrieveReliefInvestmentsValidator
          .validate(inputData)
