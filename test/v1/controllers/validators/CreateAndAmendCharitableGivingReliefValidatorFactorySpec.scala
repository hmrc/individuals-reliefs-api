...
class CreateAndAmendCharitableGivingReliefValidatorFactorySpec extends UnitSpec with JsonErrorValidators {
  ...
  private def bodyWith(charityNames1: Seq[String], charityNames2: Seq[String]) = Json.parse(s"""
      |{
      |   "giftAidPayments": {
      |     "nonUkCharities": {
  ...
  private val parsedNonUkCharities1 = NonUkCharities(Some(List(validCompanyName1)), 10.99)
  private val parsedNonUkCharities2 = NonUkCharities(Some(List(validCompanyName2)), 15.99)
  private val parsedGiftAidPayments = GiftAidPayments(Some(parsedNonUkCharities1), Some(11.99), Some(12.99), Some(13.99), Some(14.99))
  private val parsedGifts           = Gifts(Some(parsedNonUkCharities2), Some(16.99), Some(17.99))

  private val parsedBody = CreateAndAmendCharitableGivingTaxReliefBody(Some(parsedGiftAidPayments), Some(parsedGifts))

  private val validatorFactory = new CreateAndAmendCharitableGivingReliefValidatorFactory()
  ...
  "validator" should {
    "return the parsed domain object" when {
      "passed a valid request" in {
        val result: Either[ErrorWrapper, CreateAndAmendCharitableGivingTaxReliefRequestData] =
          validator(validNino, validTaxYear, validBody).validateAndWrapResult()

        result shouldBe Right(CreateAndAmendCharitableGivingTaxReliefRequestData(parsedNino, parsedTaxYear, parsedBody))
      }
      ...
      "passed a valid request with only gift payments included" in {
        ...
        result shouldBe Right(CreateAndAmendCharitableGivingTaxReliefRequestData(parsedNino, parsedTaxYear, parsedBody.copy(gifts = None)))
      }
      ...
      "passed a valid request with only gifts included" in {
        ...
        result shouldBe Right(CreateAndAmendCharitableGivingTaxReliefRequestData(parsedNino, parsedTaxYear, parsedBody.copy(giftAidPayments = None)))
      }
    }
