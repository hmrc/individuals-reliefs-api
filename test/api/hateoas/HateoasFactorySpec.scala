  ...
  "wrap" should {
    ...
    implicit object LinksFactory1 extends HateoasLinksFactory[Response, Data1] {
      override def links(appConfig: AppConfig, data: Data1): Seq[Link] =
        Seq(api.hateoas.Link(s"${appConfig.apiGatewayContext}/${data.id}", GET, "rel1"))
    }
    ...
    implicit object LinksFactory2 extends HateoasLinksFactory[Response, Data2] {
      override def links(appConfig: AppConfig, data: Data2): Seq[Link] =
        Seq(api.hateoas.Link(s"${appConfig.apiGatewayContext}/${data.id}", GET, "rel2"))
    }

    "use the response specific links" in new Test {
      hateoasFactory.wrap(response, Data1("id")) shouldBe HateoasWrapper(response, Seq(api.hateoas.Link("context/id", GET, "rel1")))
    }

    "use the endpoint HateoasData specific links" in new Test {
      hateoasFactory.wrap(response, Data2("id")) shouldBe HateoasWrapper(response, Seq(api.hateoas.Link("context/id", GET, "rel2")))
    }
  }
  ...
  "wrapList" should {
    ...
    implicit object LinksFactory extends HateoasListLinksFactory[ListResponse, Response, Data1] {
      override def itemLinks(appConfig: AppConfig, data: Data1, item: Response): Seq[Link] =
        Seq(api.hateoas.Link(s"${appConfig.apiGatewayContext}/${data.id}/${item.foo}", GET, "item"))

      override def links(appConfig: AppConfig, data: Data1): Seq[Link] =
        Seq(api.hateoas.Link(s"${appConfig.apiGatewayContext}/${data.id}", GET, "rel"))
    }

    "work" in new Test {
      hateoasFactory.wrapList(ListResponse(Seq(response)), Data1("id")) shouldBe
        HateoasWrapper(
          ListResponse(Seq(HateoasWrapper(response, Seq(api.hateoas.Link("context/id/X", GET, "item"))))),
          Seq(api.hateoas.Link("context/id", GET, "rel")))
    }
  }
