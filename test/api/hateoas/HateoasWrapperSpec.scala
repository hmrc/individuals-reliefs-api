...
class HateoasWrapperSpec extends UnitSpec {
  ...
  "HateoasWrapper writes" must {
    "place links alongside wrapped object fields" in {
      Json.toJson(HateoasWrapper(TestMtdResponse("value1", 123), Seq(api.hateoas.Link("/some/resource", GET, "thing")))) shouldBe
        Json.parse("""
      |{
      |"field1": "value1",
      |"field2": 123,
  ...
  "HateoasWrapper writesEmpty" must {
    "write links" in {
      Json.toJson(HateoasWrapper((), Seq(api.hateoas.Link("/some/resource", GET, "thing")))) shouldBe
        Json.parse("""
      |{
      |  "links" : [
      |    {
