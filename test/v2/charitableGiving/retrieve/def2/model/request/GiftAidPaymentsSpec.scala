...
import v2.charitableGiving.retrieve.def2.model.response.Def2_GiftAidPayments

class GiftAidPaymentsSpec extends UnitSpec {

  "GiftAidPayments" must {
    "read from valid JSON and return the expected object" in {
      Json
        .parse("""{
                 ...
                 |}""".stripMargin)
        .as[Def2_GiftAidPayments] shouldBe
    ...
    "treat all fields as optional" in {
      JsObject.empty.as[Def2_GiftAidPayments] shouldBe
        Def2_GiftAidPayments(
          totalAmount = None,
          oneOffAmount = None,
