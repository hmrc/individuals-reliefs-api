...
class AmendPensionsReliefsValidatorFactory {
  ...
  def validator(nino: String, taxYear: String, body: JsValue): Validator[AmendPensionsReliefsRequestData] = new Validator[AmendPensionsReliefsRequestData] {
    def validate: Validated[Seq[MtdError], AmendPensionsReliefsRequestData] = (
      ...
      resolveJson(body)
      ).mapN(AmendPensionsReliefsRequestData) andThen AmendPensionsReliefsValidator.validateBusinessRules
  }
}
